 ...
 */
import {
  propertyChange, ExecuteRule, Initialize, RemoveItem, Change, FormLoad, FieldChanged, ValidationComplete, Valid, Invalid, SubmitSuccess, CustomEvent, RequestSuccess, RequestFailure, SubmitError, Submit, Save, Reset, SubmitFailure, Focus, RemoveInstance, AddInstance, AddItem, Click,
} from './afb-events.js';
      ...
      if (item.type === 'string[]' && item?.format === 'data-url') {
        if (isAsync) {
          return item.serialize().then((serializedFiles) => {
            ret[item.id] = serializedFiles.map((x) => ({ ...x, dataRef }));
            return ret;
          });
        }
      ...
      if (item.type === 'string' && item?.format === 'data-url') {
        if (isAsync) {
          return item.serialize().then((serializedFile) => {
            ret[item.id] = { ...serializedFile[0], dataRef };
            return ret;
          });
        }
...
const matchMediaType = (mediaType, accepts) => mediaType !== '' && (!mediaType || accepts.some((accept) => {
...
class FunctionRuntimeImpl {
  ...
  getFunctions() {
    ...
    const defaultFunctions = {
      ...
      importData: {
        _func: (args, data, interpreter) => interpreter.globals.form.withDependencyTrackingControl(true, () => {
...
const transformFieldName = (fieldName) => fieldName.split('.').slice(1).map((p) => (p.match(/\[\d+\]$/) ? p : p !== '' ? `${p}[0]` : p)).join('.');
class Version {
  #minor;
...
export {
  createFormInstance, createFormInstanceSync, fetchForm, registerFunctions, restoreFormInstance, validateFormData, validateFormInstance,
};
